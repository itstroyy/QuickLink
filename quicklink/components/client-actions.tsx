...
import { Archive, CalendarClock, Eye, Pencil, Power } from 'lucide-react'
...
export default function ClientActions({ business }: { business: Business }) {
  ...
  return <div className="flex flex-wrap gap-2"><Link href={`/${business.slug}`} target="_blank" className={style}><Eye size={16}/> View page</Link><Link href={`/admin/clients/${business.id}/edit`} className={style}><Pencil size={16}/> Edit</Link><Link href={`/admin/clients/${business.id}/hub`} className={style}><CalendarClock size={16}/> Services &amp; hours</Link><button onClick={toggle} className={style}><Power size={16}/> {business.status === 'active' ? 'Disable' : 'Enable'}</button><button onClick={archive} className={`${style} text-red-650`}><Archive size={16}/> Archive</button></div>
}
