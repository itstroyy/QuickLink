      ...
      <fieldset className="client-request-group">
        ...
        <div className="client-request-grid">
          ...
          {settings.show_preferred_date&&<label className="client-field-label">Preferred date<span className={settings.preferred_date_required?'client-field-required':'client-field-optional'}>{settings.preferred_date_required?'Required':'Optional'}</span>
            <DateField value={form.preferredDate} onChange={(v)=>set('preferredDate',v)} onBlur={()=>blur('preferredDate')} min={new Date().toISOString().slice(0,10)} ariaLabel="Preferred date" placeholder="Tap to choose a date" invalid={Boolean(showError('preferredDate'))}/>
...
export function BookingModule({businessId,businessName,services,settings,paymentConfig,primary,selectedOffer,selectedServiceId,onClearOffer,standaloneServicesVisible,entrySignal}:{businessId:string;businessName:string;services:Service[];settings:BookingSettings;paymentConfig:PublicPaymentConfig;primary:boolean;selectedOffer?:SelectedBookingOffer|null;selectedServiceId?:string|null;onClearOffer?:()=>void;onSelectService?:(id:string)=>void;onClearSelectedService?:()=>void;standaloneServicesVisible?:boolean;entrySignal?:number}){
  ...
  // that exact list here — even in a compact form — reads as duplication.
  // So by default, when Services is visible AND nothing has been selected
  // yet AND the customer arrived organically (not via the "Book an
  // appointment" CTA), this step shows no service names at all — just a
  // prompt pointing at Services above with a "Choose a service here"
  // expand affordance. Selecting a service from Services (or an offer)
  // still jumps straight to "Choose a time" as before. Explicitly clicking
  // "Book an appointment" with nothing selected, or clicking "Change"/Back
  // from the time step, intentionally expands the full picker. When
  // Services is disabled, Booking stays fully self-contained and shows the
  // full list immediately, as before. None of this is specific to any one
  // business or industry — it only depends on whether standalone Services
  ...
  // off, so the remove ("x") control on an already-selected single service
  // silently re-selected the same service instead of clearing it.
  function toggle(id:string){setSelectedIds(current=>{if(current.includes(id))return current.filter(x=>x!==id);return paymentConfig.allow_multiple_services?[...current,id]:[id]})}
  ...
  return <section id="quicklink-booking" className={`client-module client-commerce ${primary?'client-module-primary':''}`}><div className="client-module-heading"><div><span className="client-module-kicker"><Calendar size={13}/>{settings.button_title||'Book now'}</span><h2>{step==='done'?'Appointment confirmed':step==='service'?'Choose services':step==='time'?'Choose a time':'Your details'}</h2></div></div>
    ...
    {step==='details'&&<div className="client-commerce-form"><input className={fieldClass} placeholder="Name" value={details.name} onChange={e=>setDetails({...details,name:e.target.value})}/><input className={fieldClass} type="tel" placeholder="Phone" value={details.phone} onChange={e=>setDetails({...details,phone:e.target.value})}/><input className={fieldClass} type="email" placeholder={online?'Email for your receipt':'Email (recommended)'} value={details.email} onChange={e=>setDetails({...details,email:e.target.value})}/><textarea className={fieldClass} rows={2} placeholder="Notes (optional)" value={details.notes} onChange={e=>setDetails({...details,notes:e.target.value})}/>
      <div className="client-booking-review rounded-xl border p-4 text-sm">
        ...
        <p className="flex justify-between"><span>Service total</span><strong>{money(total,paymentConfig.currency)}</strong></p>
        {paymentMode==='deposit'&&<><p className="mt-2 flex justify-between"><span>Deposit due now</span><strong>{money(due,paymentConfig.currency)}</strong></p><p className="mt-2 flex justify-between text-xs" style={{color:'var(--muted-text)'}}><span>Due at appointment</span><strong>{money(total-due,paymentConfig.currency)}</strong></p></>}
        {paymentMode==='full'&&<p className="mt-2 text-xs" style={{color:'var(--muted-text)'}}>Full payment is due now, securely through Stripe.</p>}
        {paymentMode==='none'&&<p className="mt-2 text-xs" style={{color:'var(--muted-text)'}}>Payment due at appointment.</p>}
        <p className="mt-3 border-t pt-3 text-xs" style={{color:'var(--muted-text)'}}>{paymentConfig.booking_refund_policy==='window'?`Refundable until ${paymentConfig.cancellation_window_hours} hours before the appointment.`:paymentConfig.booking_refund_policy==='non_refundable'?'Payments are non-refundable.':'Cancellations and refunds require business approval.'}</p>
      </div>
      {error&&<p className="client-commerce-error">{error}</p>}<div className="client-commerce-nav"><button type="button" onClick={()=>setStep('time')}><ArrowLeft size={15}/>Back</button><button type="button" disabled={busy||!details.name||!details.phone||(online&&!details.email)} onClick={submit}>{busy?'Preparing…':online?'Continue to payment':'Confirm booking'} <ArrowRight size={15}/></button></div></div>}
