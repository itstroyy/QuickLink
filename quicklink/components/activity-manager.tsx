...
export default function ActivityManager({ activity }: { activity: Activity }) {
  ...
  async function runMutation(key: string, success: string, action: () => Promise<void>) {
    if (pending.current) return
    pending.current = true; setBusy(key); setMessage('')
  ...
  async function orderStatus(id: string, value: OrderStatus) {
    await runMutation(`status-${id}`, `Order status updated to ${value}.`, async () => {
      ...
      setOrders((rows) => rows.map((row) => row.id === id ? { ...row, status: value, archived: archived || row.archived } : row))
    })
  }
  async function appointmentStatus(businessId: string, id: string, value: AppointmentStatus) {
    await runMutation(`status-${id}`, `Booking status updated to ${value}.`, async () => {
      ...
      setAppointments((rows) => rows.map((row) => row.id === id ? { ...row, status: value, archived: value === 'completed' || value === 'cancelled' ? true : row.archived, ...(value === 'cancelled' ? { external_calendar_event_id: null } : {}) } : row))
    })
  }
  async function requestStatus(id: string, value: ServiceRequest['status']) {
    await runMutation(`status-${id}`, `Request status updated to ${value.replaceAll('_', ' ')}.`, async () => {
      const archived = value === 'completed' || value === 'cancelled'
      const { error } = await supabase.from('service_requests').update({ status: value, ...(archived ? { archived: true } : {}) }).eq('id', id)
      if (error) throw error
      setRequests((rows) => rows.map((row) => row.id === id ? { ...row, status: value, archived: archived || row.archived } : row))
    })
  }
  async function setArchived(key: TabKey, id: string, archived: boolean) {
    await runMutation(`archive-${id}`, archived ? 'Activity archived.' : 'Activity restored.', async () => {
      const { error } = await supabase.from(tableFor[key]).update({ archived }).eq('id', id)
      if (error) throw error
      if (key === 'orders') setOrders((rows) => rows.map((row) => row.id === id ? { ...row, archived } : row))
      if (key === 'bookings') setAppointments((rows) => rows.map((row) => row.id === id ? { ...row, archived } : row))
  ...
  return <div className="grid gap-5">
    ...
    <div className="overflow-hidden rounded-xl border bg-white">
      ...
      <div className="divide-y">
        {visibleList.length === 0 && <p className="p-7 text-center text-sm text-[#77776f]">Nothing matches these filters.<br/><button type="button" onClick={() => { setQuery(''); setStatus(''); setArchiveFilter('all') }} className="mt-3 min-h-11 rounded-full border px-4 font-semibold">Clear filters</button></p>}
        ...
        {tab === 'orders' && (visibleList as ActivityOrder[]).map((row) => <Row key={row.id} archived={row.archived}
          ...
          status={<Status disabled={Boolean(busy)} value={row.status} values={tabs[0].statuses} onChange={(v) => orderStatus(row.id, v as OrderStatus)}/>}
          actions={<RowActions archived={row.archived} busy={Boolean(busy)} onArchive={() => setArchived('orders', row.id, !row.archived)} onDelete={() => removeRow('orders', row.business_id, row.id)}/>}
        />)}
        ...
        {tab === 'bookings' && (visibleList as ActivityAppointment[]).map((row) => <Row key={row.id} archived={row.archived}
          ...
          status={<Status disabled={Boolean(busy)} value={row.status} values={tabs[1].statuses} onChange={(v) => appointmentStatus(row.business_id, row.id, v as AppointmentStatus)}/>}
          actions={<RowActions archived={row.archived} busy={Boolean(busy)} onArchive={() => setArchived('bookings', row.id, !row.archived)} onDelete={() => removeRow('bookings', row.business_id, row.id)}/>}
        />)}
        ...
        {tab === 'requests' && (visibleList as ActivityRequest[]).map((row) => <Row key={row.id} archived={row.archived}
          ...
          status={<Status disabled={Boolean(busy)} value={row.status} values={tabs[2].statuses} onChange={(v) => requestStatus(row.id, v as ServiceRequest['status'])}/>}
          actions={<RowActions archived={row.archived} busy={Boolean(busy)} onArchive={() => setArchived('requests', row.id, !row.archived)} onDelete={() => removeRow('requests', row.business_id, row.id)}/>}
        />)}
      </div>
    </div>
...
function Row({ customer, details, status, actions, archived }: { customer: React.ReactNode; details: React.ReactNode; status: React.ReactNode; actions: React.ReactNode; archived: boolean }) {
  return <div className="ql-enter grid gap-2 px-4 py-3 text-sm md:grid-cols-[1fr_1.3fr_130px_84px] md:items-start md:gap-3">
...
function Status({ value, values, onChange, disabled }: { disabled?: boolean; value: string; values: readonly string[]; onChange: (value: string) => void }) {
  return <div className="grid gap-1.5"><StatusBadge value={value}/><select disabled={disabled} aria-busy={disabled} className={`${select} text-xs`} aria-label="Status" value={value} onChange={(e) => onChange(e.target.value)}>{values.map((item) => <option key={item} value={item}>{item[0].toUpperCase() + item.slice(1).replace('_', ' ')}</option>)}</select></div>
}
