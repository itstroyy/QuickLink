...
type HubProps = { business: Business; services: Service[]; promotions: Promotion[]; hours: BusinessHour[]; announcements: Announcement[]; gallery: GalleryItem[]; leadForms: LeadForm[]; leads: LeadSubmission[]; showHours?: boolean }
...
export default function HubManager(props: HubProps) {
  ...
  function showMutationError(context: string, action: string, error: unknown) {
    reportClientMutationError(context, error)
    setMessage(mutationErrorMessage(action, error))
  }
  async function saveRow(table: string, row: Record<string, unknown>, setter: (value: any) => void, items: Array<{ id: string }>) {
    ...
    if (error) showMutationError(`save ${table}`, `save this ${table === 'promotions' ? 'offer' : 'item'}`, error); else setter([...items.filter((item) => item.id !== row.id), data])
    setBusy('')
  }

  function deleteRow(table: string, id: string, setter: (value: any) => void, items: Array<{ id: string }>) {
    setPendingDelete({ label: 'this item', run: () => performDelete(table, id, setter, items) })
  }

  async function performDelete(table: string, id: string, setter: (value: any) => void, items: Array<{ id: string }>) {
    setBusy(`delete-${id}`)
    const { error } = await supabase.from(table).delete().eq('id', id)
    if (error) showMutationError(`delete ${table}`, 'remove this item', error); else setter(items.filter((item) => item.id !== id))
    setBusy(''); setPendingDelete(null)
  }
  ...
  async function uploadGallery(file: File) {
    ...
    if (uploadError) showMutationError('upload gallery image', 'upload this image', uploadError)
    else {
      ...
      if (error) showMutationError('create gallery item', 'save this gallery image', error); else setGallery([...gallery, data as GalleryItem])
    }
    setBusy('')
  }
  ...
  async function uploadItemImage(kind: 'service' | 'offer', id: string, file: File) {
    ...
    const { error: uploadError } = await supabase.storage.from('business-assets').upload(path, file, { cacheControl: '3600' })
    if (uploadError) { showMutationError(`upload ${kind} image`, 'upload this image', uploadError); setBusy(''); return }
    const image_url = supabase.storage.from('business-assets').getPublicUrl(path).data.publicUrl
    if (kind === 'service') {
      ...
      if (row) {
        ...
        if (error) showMutationError('save service image', 'save this service image', error); else setServices(services.map((item) => item.id === id ? next : item))
      }
    } else {
      ...
      if (row) {
        ...
        if (error) showMutationError('save offer image', 'save this offer image', error); else setPromotions(promotions.map((item) => item.id === id ? next : item))
      }
    }
    setBusy('')
  }
  ...
  async function moveService(id: string, direction: -1 | 1) {
    ...
    const a = sorted[index]; const b = sorted[swapIndex]
    const nextA = { ...a, display_order: b.display_order }; const nextB = { ...b, display_order: a.display_order }
    setServices(services.map((item) => item.id === a.id ? nextA : item.id === b.id ? nextB : item))
    setBusy(`services-${a.id}`)
    const { error } = await supabase.from('services').upsert([{ ...nextA, business_id: props.business.id }, { ...nextB, business_id: props.business.id }])
    if (error) { showMutationError('reorder services', 'reorder these services', error); setServices(services) }
    setBusy('')
  }

  function addService() { setServices([...services, { id: crypto.randomUUID(), business_id: props.business.id, name: 'New service', description: '', category: '', price_cents: null, duration_minutes: null, deposit_cents: null, image_url: null, enabled: true, bookable: true, action_type:'bookable', payment_override:null, deposit_type:null, deposit_value:null, display_order: services.length }]) }
  ...
  return <div className="grid gap-6">
    ...
    <section id="services" className={sectionStyle}><Heading title="Services" subtitle="Services are work the business performs. Choose whether each can be booked, requested as a quote, or displayed only." action={<button onClick={addService} className={buttonStyle}><Plus size={15}/> Add service</button>}/><div className="grid gap-3">{services.sort((a,b) => a.display_order-b.display_order).map((row, index, sorted) => <EditorRow key={row.id} onSave={() => saveRow('services', row as unknown as Record<string, unknown>, setServices, services)} onDelete={() => deleteRow('services', row.id, setServices, services)} busy={busy === `services-${row.id}`} extra={<><button onClick={() => moveService(row.id, -1)} disabled={index === 0} className="icon-button" aria-label={`Move ${row.name} up`}><ArrowUp size={15}/></button><button onClick={() => moveService(row.id, 1)} disabled={index === sorted.length - 1} className="icon-button" aria-label={`Move ${row.name} down`}><ArrowDown size={15}/></button></>}><label className="grid gap-1 text-xs font-semibold text-[#55554f]">Service name<input className="form-control" value={row.name} onChange={(e) => setServices(services.map((x) => x.id === row.id ? {...x,name:e.target.value}:x))} placeholder="Service name"/></label><label className="grid gap-1 text-xs font-semibold text-[#55554f]">Description<input className="form-control" value={row.description || ''} onChange={(e) => setServices(services.map((x) => x.id === row.id ? {...x,description:e.target.value}:x))} placeholder="Short description"/></label><div className="grid grid-cols-2 gap-2"><label className="grid gap-1 text-xs font-semibold text-[#55554f]">Price<input className="form-control" type="number" min="0" step="0.01" value={row.price_cents == null ? '' : row.price_cents / 100} onChange={(e) => setServices(services.map((x) => x.id === row.id ? {...x,price_cents:e.target.value ? Math.round(Number(e.target.value)*100):null}:x))} placeholder="Optional"/></label><label className="grid gap-1 text-xs font-semibold text-[#55554f]">Duration<input className="form-control" type="number" min="1" value={row.duration_minutes || ''} onChange={(e) => setServices(services.map((x) => x.id === row.id ? {...x,duration_minutes:e.target.value ? Number(e.target.value):null}:x))} placeholder="Minutes"/></label><label className="col-span-2 grid gap-1 text-xs font-semibold text-[#55554f]">Customer action<select className="form-control" value={row.action_type||'bookable'} onChange={(e)=>setServices(services.map((x)=>x.id===row.id?{...x,action_type:e.target.value as Service['action_type'],bookable:e.target.value==='bookable'}:x))}><option value="bookable">Bookable</option><option value="request_quote">Request quote</option><option value="display_only">Display only</option></select></label><label className="flex items-center gap-2 text-xs font-medium"><input type="checkbox" checked={row.enabled} onChange={(e)=>setServices(services.map((x)=>x.id===row.id?{...x,enabled:e.target.checked}:x))}/> Active</label><label className="col-span-2 flex cursor-pointer items-center gap-2 border-t pt-2 text-xs font-medium text-[#77776f]">{row.image_url&&<img src={row.image_url} alt="" className="size-8 rounded-lg object-cover"/>}<ImagePlus size={14}/>{busy===`service-image-${row.id}`?'Uploading…':row.image_url?'Replace service image':'Add service image'}<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(e)=>e.target.files?.[0]&&uploadItemImage('service',row.id,e.target.files[0])}/></label></div></EditorRow>)}{services.length === 0 && <Empty text="No services yet."/>}</div></section>
    ...
    <ConfirmationDialog open={Boolean(pendingDelete)} title="Remove item?" body={`${pendingDelete?.label || 'This item'} will be permanently removed from this business and its public page.`} confirmLabel="Remove" busy={busy.startsWith('delete-')} onCancel={()=>setPendingDelete(null)} onConfirm={()=>pendingDelete?.run()}/>
  </div>
}
...
function EditorRow({ children, onSave, onDelete, busy, extra }: { children: React.ReactNode; onSave: () => void; onDelete: () => void; busy: boolean; extra?: React.ReactNode }) { return <div className="grid gap-3 rounded-xl border border-[#e4e2da] bg-[#fafaf7] p-4 md:grid-cols-[1fr_1fr_1fr_auto]"><>{children}</><div className="flex justify-end gap-1">{extra}<button onClick={onSave} className="icon-button" aria-label="Save">{busy ? <Loader2 size={15} className="animate-spin"/> : <Save size={15}/>}</button><button onClick={onDelete} className="icon-button text-red-600" aria-label="Delete"><Trash2 size={15}/></button></div></div> }
