...
// Calendar is entirely optional — nothing here runs unless someone who can
// manage this business (platform admin, or the business's own owner/manager)
...
export async function GET(request: Request) {
  ...
  const returnTo = session.isAdmin ? `/admin/clients/${businessId}/edit` : `/dashboard/more?business=${businessId}`
  ...
  response.cookies.set('quicklink_google_oauth', JSON.stringify({ state, businessId, returnTo }), {
    httpOnly: true,
    sameSite: 'lax',
