...
export default function ClientActivityManager({ slug, token, access, initialOrders, initialAppointments, initialRequests }: { slug: string; token: string; access: BusinessClientAccess; initialOrders: CustomerOrder[]; initialAppointments: Appointment[]; initialRequests: ServiceRequest[] }) {
  ...
  async function mutate(id: string, action: 'status' | 'archive', value: string | boolean) {
    if (pending.current) return
    pending.current = true; setBusy(`${action}-${id}`); setMessage('')
    const currentTab = tab
    try {
      const response = await fetch(`/api/client-activity/${action}`, {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ slug, token, type: currentTab, id, ...(action === 'status' ? { status: value } : { archived: value }) }),
      })
      const result = await response.json()
      if (!response.ok) throw new Error(result.error || 'Unable to update activity.')
      // Apply only known, server-confirmed statuses; terminal records retain existing archive semantics.
      if (action === 'status' && typeof value === 'string') {
        const terminal = value === 'completed' || value === 'cancelled'
        if (currentTab === 'orders' && isStatus(value, tabs[0].statuses)) setOrders((rows) => rows.map((row) => row.id === id ? { ...row, status: value, archived: terminal || row.archived } : row))
        if (currentTab === 'bookings' && isStatus(value, tabs[1].statuses)) setAppointments((rows) => rows.map((row) => row.id === id ? { ...row, status: value, archived: terminal || row.archived } : row))
        if (currentTab === 'requests' && isStatus(value, tabs[2].statuses)) setRequests((rows) => rows.map((row) => row.id === id ? { ...row, status: value, archived: terminal || row.archived } : row))
      }
      // Confirmed mutations update locally even if the following feed refresh fails.
      if (action === 'archive') {
        const archived = value === true
        if (currentTab === 'orders') setOrders((rows) => rows.map((row) => row.id === id ? { ...row, archived } : row))
        if (currentTab === 'bookings') setAppointments((rows) => rows.map((row) => row.id === id ? { ...row, archived } : row))
        if (currentTab === 'requests') setRequests((rows) => rows.map((row) => row.id === id ? { ...row, archived } : row))
      }
      notify(action === 'status' ? `Status updated to ${String(value).replaceAll('_', ' ')}.` : value ? 'Activity archived.' : 'Activity restored.')
      await refresh()
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not update activity. Please try again.') }
    finally { pending.current = false; setBusy('') }
  }
  async function updateStatus(id: string, value: string) { await mutate(id, 'status', value) }
  async function setArchived(id: string, archived: boolean) { await mutate(id, 'archive', archived) }
...
function isStatus<T extends string>(value: string, allowed: readonly T[]): value is T { return (allowed as readonly string[]).includes(value) }
...
function ActivityRow({archived,customer,details,children}:{archived:boolean;customer:React.ReactNode;details:React.ReactNode;children:React.ReactNode}){return <div className="ql-enter grid gap-3 p-4 md:grid-cols-[1fr_1.5fr_150px_auto]"><div>{archived&&<span className="mb-1 inline-block rounded-full bg-[#f0eee7] px-2 py-0.5 text-[10px] font-semibold uppercase text-[#77776f]">Archived</span>}{customer}</div><div>{details}</div>{children}</div>}
