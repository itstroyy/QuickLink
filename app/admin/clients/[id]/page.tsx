...
import { getPublicSiteUrl, throwSupabaseError } from '@/lib/supabase/config'

export default async function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const supabase = await createClient()
  const [businessResult, linksResult, views, clicks, events] = await Promise.all([
  ...
  if (businessError) throwSupabaseError('admin-client-detail', businessError, { businessId: id })
  if (linksError) throwSupabaseError('admin-client-links', linksError, { businessId: id })
  if (views.error) throwSupabaseError('admin-client-view-count', views.error, { businessId: id })
  if (clicks.error) throwSupabaseError('admin-client-click-count', clicks.error, { businessId: id })
  if (events.error) throwSupabaseError('admin-client-click-events', events.error, { businessId: id })
  ...
  const publicUrl = `${getPublicSiteUrl()}/${typedBusiness.slug}`
